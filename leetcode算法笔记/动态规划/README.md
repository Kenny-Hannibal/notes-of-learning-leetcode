...
如上图所示，为暴力递归求斐波那契数 f(5)f(5) 形成的二叉树，树中的每个节点代表着执行了一次 `fibonacci() `函数，且有：
...
* 子问题： f(n)f(n) 的子问题包括 f(0), f(1), f(2), \cdots, f(n - 1)f(0),f(1),f(2),⋯,f(n−1) ，分别代表重量为 0, 1, 2, \cdots, n - 10,1,2,⋯,n−1 蛋糕的最高售价。 已知无蛋糕时 f(0) = 0f(0)=0 ，蛋糕重量为 1 时不可切分 f(1) = p(1)f(1)=p(1) ；
