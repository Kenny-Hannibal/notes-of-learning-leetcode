...
class WordBasedTokenizer(object):
    ...
    def _find_all_words(self, sentence, sentence_length):
        ...
        """
        graph = WordGraph()
        all_words = {0: [0], sentence_length - 1: [sentence_length - 1]}   # 先初始化all_words字典，将开始节点和结束节点加进来
        graph.insert_start_word(WordGraph.NODE_S)   # 将开始节点添加进来
        ...
        for i in range(1, sentence_length - 1):
            ...
            # 向后扫描，看是否成词
            j = i + 1  # 词尾位置
            fragment = ''.join(sentence[i:j + 1])  # 在词库中寻找，以当前节点为起始节点，后续的在词库中所存在的真实的词组
    ...
    def seg(self, sentence):
        ...
        # 前面根据标点符号进行分句后，这里则将每个分句都拆成单独的每个字，并将其装进列表内，并且在前后增加开始和结束的节点
        # 构建词图
